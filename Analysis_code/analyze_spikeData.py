...
def getAll_events(DATA):
    ...
    for i in range(len(DATA['comments']['Comment'])):        
        ...
        if f[0:6] == "rwdTL:":
            Threshold_low = int(f[6:9]) 
        if f[0:6] == "rwdTU:":
            Threshold_high = int(f[6:9])
    ...
    return Frames, Reward, Licks, StartTime, TrialStart, Gain, Frot, PosUnits, NegUnits, fmaxpos, fmaxneg, latency, RewardUp, RewardLow,  EnteringEvent, FreeWater, Threshold_high, Threshold_low
...
def reconstruct_trajectory(TrialStart, Separate, PosUnits, NegUnits, fmaxpos, fmaxneg, gain, frot, sec):
    ...
    for trial in range(len(TrialStart)-1):
        if(TrialStart[trial+1]-TrialStart[trial])>length:
            length=TrialStart[trial+1]-TrialStart[trial]
            ind_debut=trial
            ind_fin=trial+1
    ...
    for ft in range(len(Frames[mouse][session][:][:,1])):
        if Frames[mouse][session][:][:,1][ft]>TrialStart[ind_debut]:
            CurrentPosition=Frames[mouse][session][:][:,0][ft]
            break
    

    

    for time in range((TrialStart[ind_debut]),(TrialStart[ind_fin]-30000*(25-sec)),30):  
        for j in range(len(PosUnits)):
            timepos=0
            if Separate[PosUnits[j][0]][PosUnits[j][1]] != []: 
                while (Separate[PosUnits[j][0]][PosUnits[j][1]][timepos]) < (TrialStart[ind_fin]-30000*(25-sec)):
                    if time-30<Separate[PosUnits[j][0]][PosUnits[j][1]][timepos]<=time:
                        CurrentPosition+=float(4*gain)*float(fmaxneg)
                    timepos+=1
                    if timepos>=len(Separate[PosUnits[j][0]][PosUnits[j][1]]):
                        break
        ...
        for k in range(len(NegUnits)):
            ...
            if Separate[NegUnits[k][0]][NegUnits[k][1]] != []:
                ...
                while (Separate[NegUnits[k][0]][NegUnits[k][1]][timeneg]) < (TrialStart[ind_fin]-30000*(25-sec)):
                    if time-30<Separate[NegUnits[k][0]][NegUnits[k][1]][timeneg]<=time:
                        CurrentPosition-=float(4*gain)*float(fmaxpos)
                    timeneg+=1
                    if timeneg>=len(Separate[NegUnits[k][0]][NegUnits[k][1]]):
                        break
                   
        #CurrentSpeed = (PreviousSpeed/frot) + 1/1000.*CurrentAcceleration
...
def draw_clock(pen):
    ...
    for degree in range(600,1200,1):
        pen.color("chartreuse")
        pen.up()
        pen.goto(0,0)
        pen.setheading(degree/10)
        pen.fd(32)
        pen.down()
        pen.fd(177)
        
    for degree in range(-1800,0,1):
        pen.color("tomato")
        pen.up()
        pen.goto(0,0)
